    ...
    const mapPicker = ($wire, config, state) => {
        return {
            ...
            createMap: function (el) {
                ...
                if (!config.isEditing && config.liveLocation.send && config.liveLocation.realtime) {
            ...
            initFormRestoration: function () {
                ...
                window.addEventListener("pageshow", (event) => {
                    // Force set formRestorationHiddenInput to current location
                    if(!config.isEditing && config.liveLocation.send && config.liveLocation.realtime){
                        this.formRestorationHiddenInput.value = JSON.stringify({lat: this.currentCoordinates.lat, lng: this.currentCoordinates.lng, zoom: config.controls.zoom})
                    }
            ...
            fetchCurrentLocation: function () {
                if ('geolocation' in navigator) {
                    navigator.geolocation.getCurrentPosition(async position => {
                        ...
                        if(!config.isEditing){
                            config.default.lat = this.currentCoordinates.lat;
                            config.default.lng = this.currentCoordinates.lng;    
                        }
                        // Ensure map is ready before flying
                        if (this.map && this.map._loaded) {
                            this.map.flyTo(currentPosition);
                            this.updateLocation();
                            this.updateMarker();
                        } else {
                            // Fallback: Set center manually if flyTo fails
                            this.map.setView(currentPosition);
                            this.updateLocation();
                            this.updateMarker();
                        }
                    }, error => {
